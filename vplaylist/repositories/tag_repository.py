...
class SqliteTagRepository(TagRepository):
    ...
    def get_all(self) -> list[Tag]:
        ...
        tags = [
            Tag(uuid=i[0], name=i[1], note=i[2], tags=[]) for i in cursor.fetchall()
        ]
        conn.close()
        return tags
    ...
    def search(self, search: str) -> list[Tag]:
        ...
        tags = [Tag(uuid=i[0], name=i[1], note=i[2]) for i in cursor.fetchall()]
        conn.close()
        return tags
    ...
    def add_tag(self, name: str) -> Tag:
        ...
        conn.close()
        return Tag(uuid=query_result[0], name=query_result[1], note=query_result[2])
    ...
    def add_video_tag_relation(self, video_uuid: UUID, tag_uuid: UUID) -> bool:
        ...
        insert_video_tag_query = """
            ...
            VALUES (?,?)
        """
        conn.execute(insert_video_tag_query, (str(video_uuid), str(tag_uuid)))
        conn.commit()
        conn.close()
        return True

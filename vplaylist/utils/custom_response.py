...
class VideoStreamResponse(StreamingResponse):
    ...
    def _chunk_generator_from_stream(self, stream, chunk_size, start, size):
        bytes_read = 0
        stream.seek(start)
    ...
    def __init__(self, video: PlayableVideo, range_asked: str | None):
        ...
        start_byte_requested = 0 if not range_asked else int(range_asked.split("=")[-1][:-1])
        end_byte_planned = min(start_byte_requested + self.BYTES_PER_RESPONSE, total_size)

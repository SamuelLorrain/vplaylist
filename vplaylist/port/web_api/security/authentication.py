...
from vplaylist.services.authentication_service import is_token_valid, get_username_from_token
...
def authorize_video_uuid(uuid: UUID, authorization: Annotated[Optional[str], Header()] = None) -> UUID:
    video_repository = VideoRepository()
    account_repository = AccountRepository()
    account = get_account(authorization)
    if video_repository.video_is_in_rootpath(uuid, account_repository.get_rootpath_for_account(account)):
        return uuid
